...
#define TARGET_LOOP_TIME_US 1000U // 1ms target loop time
...
bool spi2_master_start_transfer(uint32_t tx_buf[SPI_BURST_WORDS], bool check_timeout, uint32_t loop_start);
bool spi2_master_finish_transfer(uint32_t rx_buf[SPI_BURST_WORDS], bool check_timeout, uint32_t loop_start);
bool spi2_master_transfer_blocking(uint32_t tx_buf[SPI_BURST_WORDS], uint32_t rx_buf[SPI_BURST_WORDS], bool check_timeout, uint32_t loop_start);

// fatal state
void enter_fatal_state() {
    gpio_write(IN1_GPIO_OUT, 0);
...
int main() {
    ...
    while (!got_pong) {
        ...
        tx_buf[1] = 0xDC;
        tx_buf[2] = 0xBE;
        spi2_master_transfer_blocking(tx_buf, rx_buf, false, 0);
        if (rx_buf[0] == 0xAC && rx_buf[1] == 0xDC && rx_buf[2] == 0xBE) {
            got_pong = true;
        }
        else {
            SDK_DelayAtLeastUs(100000, DEFAULT_SYSTEM_CLOCK); // delay 100ms if pong not received correctly
        }
    }
    ...
    while (1) {
        ...
        tx_buf[1] = last_in;
        tx_buf[2] = had_fault;
        success = spi2_master_start_transfer(tx_buf, true, loop_start);
        if (!success) {
            enter_fault_state();
        }
        else {
        ...
        // finish SPI transfer
        success = spi2_master_finish_transfer(rx_buf, true, loop_start);
        if (!success) {
            enter_fault_state();
        }
        else {
        ...
        uint32_t slave_crc = rx_buf[0];
        uint32_t slave_last_in = rx_buf[1];
        uint32_t slave_had_fault = rx_buf[2];

        // evaluate
        if (had_fault == 1 || had_fault != slave_had_fault) {
        ...
        else if ((slave_crc != 0xB8) || (last_in != slave_last_in)) {
        ...
        if (first_run) {
            SDK_DelayAtLeastUs(1100, DEFAULT_SYSTEM_CLOCK);
            first_run = false;
        }
        ...
        if (gpt1_timeout_reached(loop_start)) {
            ...
            while ((GPT1->CNT - loop_start) < TARGET_LOOP_TIME_US) {
                // wait until target loop time is achieved
            }
        }
        ...
        if (in_fault_state) {
            ...
            // ping to establish communication again
            got_pong = false;
            while (!got_pong) {
                spi2_clear_buffers();
                tx_buf[0] = 0xAC;
...
bool gpt1_timeout_reached(uint32_t loop_start) {
    return (GPT1->CNT - loop_start) > TARGET_LOOP_TIME_US;
}
...
void gpio_init_all() {
    ...
    gpio_init(IN1_GPIO_OUT, true, 0U); // set GPIO3 IO20 as output (safe digital input)
    gpio_init(OUT1_GPIO_OUT, true, 0U); // set GPIO4 IO1 as output (safe digital output)
}
...
void spi2_master_init() {
    ...
    ECSPI2->CONREG =
        ECSPI_CONREG_SMC(1U) | // burst is immediately sent when data arrives in TXFIFO
        ECSPI_CONREG_CHANNEL_MODE(1U) | // master mode on channel 0
        ECSPI_CONREG_PRE_DIVIDER(5U - 1U) | // pre divider 5
        ECSPI_CONREG_POST_DIVIDER(4U) | // post divider 4 (2^4 = 16), 80MHz / 5 / 16 = 1MHz baudrate
    ...
    ECSPI2->PERIODREG |=
        ECSPI_PERIODREG_SAMPLE_PERIOD(0U) | // 0 wait states inserted
        ECSPI_PERIODREG_CSRC(0U) | // use spi clock
        ECSPI_PERIODREG_CSD_CTL(0U); // 0 spi clocks between chip select active and first clock edge
...
bool spi2_master_transfer_blocking(uint32_t tx_buf[SPI_BURST_WORDS], uint32_t rx_buf[SPI_BURST_WORDS], bool check_timeout, uint32_t loop_start) {
    if (!spi2_master_start_transfer(tx_buf, check_timeout, loop_start)) {
        return false;
    }
    if (!spi2_master_finish_transfer(rx_buf, check_timeout, loop_start)) {
        return false;
    }
    return true;
}
